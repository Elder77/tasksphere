...
export class IdentifiersService {
  ...
  async create(dto: CreateIdentifierDto) {
    ...
    const exists = await this.prisma.ticket_identificador.findFirst({ where: { tiid_nombre: dto.tiid_nombre, tipr_id: dto.tipr_id } });
    if (exists) {
      // return field-specific error so frontend shows it inline
      throw new BadRequestException({ tiid_nombre: ['Identificador con ese nombre ya existe'] });
    }
  ...
  async findAll(q?: string) {
    const where: any = {};
    if (q && String(q).trim()) {
      where.tiid_nombre = { contains: String(q).trim(), mode: 'insensitive' };
    }
    return this.prisma.ticket_identificador.findMany({ where, orderBy: { fecha_sistema: 'desc' } });
  }
  ...
  async findAllPaged(page = 1, perPage = 10, q?: string) {
    ...
    const where: any = {};
    if (q && String(q).trim()) where.tiid_nombre = { contains: String(q).trim(), mode: 'insensitive' };
    const [total, data] = await Promise.all([
      this.prisma.ticket_identificador.count({ where }),
      this.prisma.ticket_identificador.findMany({ where, skip: (p - 1) * pp, take: pp, orderBy: { fecha_sistema: 'desc' } }),
    ]);
