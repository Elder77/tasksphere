import { Controller, Get, Post, Body, Put, Param, Delete, UseGuards, Request, UnauthorizedException, Query } from '@nestjs/common';
...
export class TicketProyectosController {
  ...
  @ApiBearerAuth('access-token')
    @Get(':id/users')
    async getUsers(@Request() req, @Param('id') id: string) {
      if (req.user && req.user.project_token) {
        throw new UnauthorizedException('No autorizado con token de proyecto');
      }
  ...
  @ApiBearerAuth('access-token')
    @Post(':id/users')
    async setUsers(@Request() req, @Param('id') id: string, @Body() body: { usua_cedulas?: string[] }) {
      if (req.user && req.user.project_token) {
        throw new UnauthorizedException('No autorizado con token de proyecto');
      }
  ...
  @ApiBearerAuth('access-token')
    @Post()
    create(@Request() req, @Body() body: CreateProyectoDto) {
      // Disallow creating projects when the token used is a project token (project_token===true)
      if (req.user && req.user.project_token) {
        throw new UnauthorizedException('No autorizado a crear proyectos con token de proyecto');
      }
  ...
  @ApiBearerAuth('access-token')
    @Put(':id')
    update(@Request() req, @Param('id') id: string, @Body() body: UpdateProyectoDto) {
      if (req.user && req.user.project_token) {
        throw new UnauthorizedException('No autorizado con token de proyecto');
      }
