...
export class TicketCategoriasService {
  ...
  findAll(q?: string) {
    ...
    return this.prisma.ticket_categorias.findMany({ where, orderBy: { fecha_sistema: 'desc' } });
  }
  ...
  async findAllPaged(page = 1, perPage = 10, q?: string) {
    ...
    const where: any = {};
    if (q && String(q).trim()) where.tica_nombre = { contains: String(q).trim(), mode: 'insensitive' };
    const [total, data] = await Promise.all([
      this.prisma.ticket_categorias.count({ where }),
      this.prisma.ticket_categorias.findMany({ where, skip: (p - 1) * pp, take: pp, orderBy: { fecha_sistema: 'desc' } }),
    ]);
    return { data, meta: { total, page: p, perPage: pp, totalPages: Math.ceil(total / pp) } };
  }

  async findOne(tica_id: number) {
    const p = await this.prisma.ticket_categorias.findUnique({ where: { tica_id } });
    if (!p) throw new NotFoundException('Categoría no encontrada');
    return p;
  }

  create(dto: CreateCategoriaDto) {
    return this.prisma.ticket_categorias.create({ data: { tica_nombre: dto.tica_nombre, tica_estado: dto.tica_estado ?? 'A' } });
  }

  update(tica_id: number, dto: UpdateCategoriaDto) {
    return this.prisma.ticket_categorias.update({ where: { tica_id }, data: dto });
  }
