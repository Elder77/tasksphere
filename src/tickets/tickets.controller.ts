...
export class TicketsController {
  ...
  @ApiOperation({
    ...
    description:
      'Devuelve una lista paginada de tickets. Parámetros: page, perPage, status, module, search.',
  })
  ...
  @ApiResponse({
    status: 200,
    description: 'Lista de tickets devuelta correctamente',
    schema: {
      example: {
        data: [
          {
            tick_id: 1,
            tick_nombre: 'Problema X',
            tick_estado: 'P',
  ...
  @ApiOperation({
    ...
    description:
      'Devuelve el ticket indicado por ID junto a sus archivos y el historial.',
  })
  ...
  @ApiOperation({
    ...
    description:
      'Crear un nuevo ticket. Se puede subir hasta 3 archivos (png, jpg, pdf, doc/docx, xls/xlsx) con máximo 5MB cada uno.',
  })
  ...
  @ApiBody({
    schema: {
      ...
      properties: {
        tiid_id: { type: 'number' },
        tick_nombre: { type: 'string' },
        tick_descripcion: { type: 'string' },
        tick_modulo: { type: 'string' },
        usua_cedula: { type: 'string' },
        attachments: {
          type: 'array',
  ...
  @ApiResponse({
    ...
    schema: {
      example: {
        tick_id: 10,
        tick_nombre: 'Prueba e2e',
        tick_descripcion: 'Descripción prueba',
        tick_id_identificador: 1,
        tick_modulo: 'Soporte',
      },
    },
  ...
  @ApiResponse({
    status: 400,
    description: 'Datos inválidos o archivos no permitidos (tipo o tamaño)',
  })
  @ApiResponse({
    status: 401,
    description: 'No autorizado. Token inválido o no provisto.',
  ...
  @ApiOperation({
    ...
    description:
      'Editar descripción ó módulo y agregar archivos (hasta 3). No se puede cambiar el identificador.',
  })
  @ApiConsumes('multipart/form-data')
  @ApiBody({
    schema: {
  ...
  @ApiResponse({
    ...
    schema: {
      example: {
        tick_id: 10,
        tick_nombre: 'Prueba e2e',
        tick_descripcion: 'Descripción actualizada',
        tick_modulo: 'Soporte',
      },
    },
  })
  @ApiResponse({
    status: 400,
    description: 'Datos inválidos o archivos no permitidos',
  })
  @ApiResponse({
    status: 401,
    description: 'No autorizado. Token inválido o no provisto.',
  ...
  @ApiResponse({
    status: 403,
    description: 'Permisos insuficientes para editar este ticket',
  })
  @UseInterceptors(FilesInterceptor('attachments', 3))
  update(
    @Req() req: unknown,
  ...
  @ApiOperation({
    ...
    description:
      'Asignar el ticket a un usuario. Sólo administradores (perf_id=2) pueden realizarlo.',
  })
  ...
  assign(@Req() req: unknown, @Body() body: AssignTicketDto) {
    ...
    return this.ticketsService.assign(
      ...
      String(body.tick_usuario_asignado),
      body.tica_id ?? null,
      body.prio_id ?? null,
      user,
    );
  ...
  @ApiOperation({
    ...
    description:
      'Cerrar un ticket. Sólo administradores (perf_id=2) pueden cerrar. Se puede adjuntar evidencia (archivos).',
  })
  @UseInterceptors(FilesInterceptor('attachments', 3))
  ...
  @ApiOperation({
    ...
    description:
      'Reabrir un ticket. Sólo el creador puede reabrir y debe indicar la razón.',
  })
  ...
  @ApiResponse({
    status: 403,
    description: 'Solo el creador puede reabrir el ticket',
  })
  @UseInterceptors(FilesInterceptor('attachments', 3))
  reopen(
    @Req() req: unknown,
  ...
  @ApiBearerAuth('access-token')
  @ApiOperation({
    summary: 'Descargar archivo de un ticket',
    description: 'Descarga un archivo adjunto del ticket si tienes permisos.',
  })
  ...
  @ApiResponse({
    status: 403,
    description: 'Permisos insuficientes para descargar este archivo',
  })
