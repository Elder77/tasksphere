...
export class TicketsController {
  ...
  @ApiResponse({ status: 200, description: 'Lista de tickets devuelta correctamente', schema: { example: { data: [{ id: 1, title: 'Problema X', status: 'open', identifierId: 1, module: 'Soporte' }], meta: { total: 1, page: 1, perPage: 10, totalPages: 1 } } } })
  ...
  @ApiBody({ schema: { type: 'object', properties: { identifierId: { type: 'number' }, title: { type: 'string' }, description: { type: 'string' }, module: { type: 'string' }, attachments: { type: 'array', items: { type: 'string', format: 'binary' } } } } })
  @ApiResponse({ status: 201, description: 'Ticket creado correctamente', schema: { example: { id: 10, title: 'Prueba e2e', description: 'Descripción prueba', identifierId: 1, module: 'Soporte' } } })
  ...
  @ApiConsumes('multipart/form-data')
  @ApiBody({ schema: { type: 'object', properties: { description: { type: 'string' }, module: { type: 'string' }, attachments: { type: 'array', items: { type: 'string', format: 'binary' } } } } })
  @ApiResponse({ status: 200, description: 'Ticket actualizado correctamente', schema: { example: { id: 10, title: 'Prueba e2e', description: 'Descripción actualizada', module: 'Soporte' } } })
  ...
  @Post(':id/assign')
  @ApiBearerAuth('access-token')
  @ApiOperation({ summary: 'Asignar ticket', description: 'Asignar el ticket a un usuario. Sólo rol superadmin puede realizarlo.' })
  @ApiResponse({ status: 200, description: 'Ticket asignado correctamente' })
  @ApiResponse({ status: 400, description: 'ID inválido' })
  @ApiResponse({ status: 401, description: 'No autorizado. Token inválido o no provisto.' })
  @ApiResponse({ status: 403, description: 'Solo superadmin puede asignar tickets' })
  assign(@Req() req: any, @Param('id') id: string, @Body() body: any) {
    const user: AuthUser = req.user;
    return this.ticketsService.assign(+id, Number(body.assignedToId), user);
  }

  @UseGuards(JwtAuthGuard)
  @Post(':id/close')
  @ApiBearerAuth('access-token')
  @ApiOperation({ summary: 'Cerrar ticket', description: 'Cerrar un ticket. Sólo superadmin puede cerrar. Se puede adjuntar evidencia (archivos).' })
  @UseInterceptors(FilesInterceptor('attachments', 3))
  @ApiResponse({ status: 200, description: 'Ticket cerrado correctamente' })
  @ApiResponse({ status: 400, description: 'Datos inválidos' })
  @ApiResponse({ status: 401, description: 'No autorizado. Token inválido o no provisto.' })
  @ApiResponse({ status: 403, description: 'Solo superadmin puede cerrar tickets' })
