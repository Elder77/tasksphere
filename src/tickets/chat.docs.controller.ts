...
export class ChatDocsController {
  @Post('join-example')
  @ApiOperation({ summary: 'Ejemplo: unirse a una sala de ticket (WebSocket)', description: 'Este endpoint solo devuelve un ejemplo de payload para el evento `join_ticket` que se debe enviar vía WebSocket al namespace `/ws/tickets`. El cliente debe incluir el JWT en la handshake (ver descripción).' })
  ...
  @ApiOperation({ summary: 'Ejemplo: enviar mensaje (WebSocket)', description: 'Devuelve un ejemplo de cómo debe ser el mensaje enviado por el evento `message` y la respuesta persistida. En la conexión WebSocket el token JWT debe pasarse en `auth.token` (socket.io) o en query string `?token=...`.' })
